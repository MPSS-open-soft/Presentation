...
def generate_pdf_revenue_sheet():
    ...
    doc = SimpleDocTemplate(full_output_file_path,pagesize=letter)
    table = Table(data)
    style = TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.gray),
                        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
    ...
    table.setStyle(style)
    doc.build([table])
    os.system(f"start {full_output_file_path}")  # Works on Windows    
    return "sucess"  
...
def send_orders():
    ...
    for part in parts:
        if part['quantity'] < part['threshold']:
            row = [part["name"], part["_id"], part["address"],(part["threshold"] - part["quantity"]) * part['price'], part["threshold"] - part["quantity"]]
            data.append(row)
